...
public class LockedDoor : InteractTrigger
{
    ...
    [Header("钥匙")]
    [Tooltip("设了 = 查背包里有没有这件钥匙道具;留空 = 用 LevelKeyManager 钥匙计数")]
    ...
    // 设了 requiredKeyItem → 查背包有没有这件钥匙道具;否则回退到 LevelKeyManager 钥匙计数
    bool HasKey()
    {
        if (requiredKeyItem != null)
            return InventorySystem.Instance != null && InventorySystem.Instance.items.Contains(requiredKeyItem);
        return LevelKeyManager.Instance != null && LevelKeyManager.Instance.HasRequiredKeys();
    }
