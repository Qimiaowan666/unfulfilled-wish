...
{
    public string id;
    public bool defeated;   // 只记死没死;没死透的怪读档满血重置(魂类惯例),不存血量
}
...
public class SaveSystem : MonoBehaviour
{
    ...
    void ApplySceneState(SaveData data, bool realLoad)
    {
        ...
        ApplyEnemyStates(data.enemyStates, realLoad);
    ...
    void ApplyEnemyStates(EnemySaveData[] enemyStates, bool realLoad)
    {
        // realLoad(继续 / 重生 / 手动读档):清空并从存档重建"已击败的 permadeath 怪"。
        // 普通过门切场景:保留内存集合(和门一样内存连续)→ 杀过的精英/boss 过场景不复活;唯有死亡/读档才按存档回滚。
        if (realLoad)
        {
            runtimeDefeatedEnemyIDs.Clear();
            if (enemyStates != null)
                foreach (var state in enemyStates)
                    if (state != null && state.defeated && !string.IsNullOrWhiteSpace(state.id))
                        runtimeDefeatedEnemyIDs.Add(state.id);
        }

        // 把"已击败集合"应用到本场景的 permadeath 怪(普通怪由上面 RefreshRespawnableEnemies 复活,不在此处理)。
        foreach (var enemy in FindObjectsByType<EnemyBase>(FindObjectsInactive.Include))
        {
            if (enemy == null || !enemy.SavesPermanentDeath) continue;
            bool defeated = runtimeDefeatedEnemyIDs.Contains(enemy.SaveID);
            enemy.LoadSaveState(defeated);
        }
    }
