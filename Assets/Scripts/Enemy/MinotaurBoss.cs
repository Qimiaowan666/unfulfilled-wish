...
public class MinotaurBoss : EnemyBase
{
    ...
    IEnumerator Phase2TransitionRoutine()
    {
        ...
        // 3.6) 爆炸声放完 → 强化吼，吼完才解冻双方
        float roarLen = AudioManager.Instance?.PlayBossRageRoar() ?? 0f;
        yield return new WaitForSeconds(roarLen);

        // 5) 恢复 + 血条 + 玩家回神，带强化继续战斗
        if (playerBar != null) playerBar.SetHudHidden(false);
        if (bossBar   != null) bossBar.SetHidden(false);
        if (pc != null && pc.stateMachine.currentState == pc.knockedState)
            pc.stateMachine.ChangeState(pc.IsGrounded ? (PlayerBaseState)pc.idleState : pc.fallState);
        Invincible = false;
    ...
    // 兜底：boss 被禁用（场景卸载 / 死亡收尾）时清掉过渡标志 + 复原 timeScale（避免顿帧中被禁用导致新场景卡慢放）
    void OnDisable()
    {
        if (PhaseTransition) Time.timeScale = 1f;
        PhaseTransition = false;
    }
