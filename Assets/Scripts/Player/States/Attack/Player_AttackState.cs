...
public class Player_AttackState : PlayerBaseState
{
    ...
    const float FallbackDuration = 2f;   // 动画 AnimationFinish 事件漏触发时的兜底超时（正常永远不触发）

    public Player_AttackState(PlayerController player, StateMachine sm)
    ...
    public override void Enter()
    {
        ...
        stateTimer = FallbackDuration;   // 动画事件漏触发时的兜底超时（正常靠 AnimationFinish 退出）
    ...
    public override void Update()
    {
        ...
        // 攻击态内部时又按了攻击 Queue next combo on attack press
        if (input.AttackPressed && comboStep < player.maxComboStep - 1)
            comboQueued = true;
